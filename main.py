from flask import Flask, render_template, request, redirect as rd, url_for
...
def submit():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        sql = 'INSERT INTO input(name, birth,contact,dad,mom,ranking,`from`,gender) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)'
...
def chinga():
        ...
        # 데이터베이스에서 데이터 가져오기
        sql = 'SELECT * FROM input where `from`="친가"'
...
def wega():
        ...
        # 데이터베이스에서 데이터 가져오기
        sql = 'SELECT * FROM input where `from`="외가"'
        cursor.execute(sql) #sql변수의 쿼리 실행
...
def chuga():
        ...
        # 데이터베이스에서 데이터 가져오기
        sql = 'SELECT * FROM input where `from`="처가"'
        cursor.execute(sql) #sql변수의 쿼리 실행
        we_result = cursor.fetchall() #쿼리 실행 결과 받아옴
...
def chuwega():
        ...
        # 데이터베이스에서 데이터 가져오기
        sql = 'SELECT * FROM input where `from`="처외가"'
        cursor.execute(sql) #sql변수의 쿼리 실행
